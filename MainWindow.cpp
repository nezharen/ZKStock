	...
	if (!error)
	{
		...
		while (true)
		{
			...
			if (stocks->at(ord).priceMoreThan < list.at(3).toDouble())
			{
				...
				system((tr("notify-send \"ZKStock\" \"") + list.at(0) + tr("'s price is more than %1").arg(stocks->at(ord).priceMoreThan) + tr("\"")).toUtf8().data());
				(*stocks)[ord].priceMoreThan += 0.1;
			}
			ord++;
		}
	}
